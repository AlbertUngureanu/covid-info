  ...
  if(msg.author.id === '525335062464561162')
  {
    ...
    if(iD)
    {
      setTimeout(async function(){
        var em = await cazuriConfirmate(cH);
        cH.messages.fetch(iD).then(m =>m.delete());
        cH.send(em).then(m => iD = m.id);
        console.log(`Am pornit pe ${msg.guild.name}!`);
        var dayM = 1000 * 60 * 60 * 24;
        setInterval(async function(){
            let em = await cazuriConfirmate(cH);
            cH.messages.fetch(iD).then(m =>m.delete());
            cH.send(em).then(m => iD = m.id);
            console.log(`Am dat update pe ${msg.guild.name}!`);
        }, dayM)
      }, timeLeft())
    }
    else
